...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    ...
    ./kaldi-master/src/featbin/compute-mfcc-feats $vtln_opts --verbose=2 --sample-frequency=48000 --config=$mfcc_config ark:- ark:- \| \
    ./kaldi-master/src/featbin/copy-feats --compress=$compress $write_num_frames_opt ark:- \
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    ./kaldi-master/src/featbin/compute-mfcc-feats  $vtln_opts --verbose=2 --sample-frequency=48000 --config=$mfcc_config \
     scp,p:$logdir/wav_${name}.JOB.scp ark:- \| \
      ./kaldi-master/src/featbin/copy-feats $write_num_frames_opt --compress=$compress ark:- \
      ark,scp:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp \
      || exit 1;
fi

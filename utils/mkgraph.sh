...
N=$(./kaldi-master/src/bin/tree-info $tree | grep "context-width" | cut -d' ' -f2) || { echo "Error when getting context-width"; exit 1; }
P=$(./kaldi-master/src/bin/tree-info $tree | grep "central-position" | cut -d' ' -f2) || { echo "Error when getting central-position"; exit 1; }
...
if [[ ! -s $lang/tmp/LG.fst || $lang/tmp/LG.fst -ot $lang/G.fst || \
  ...
  ./kaldi-master/src/fstbin/fsttablecompose $lang/L_disambig.fst $lang/G.fst | ./kaldi-master/src/fstbin/fstdeterminizestar --use-log=true | \
    ./kaldi-master/src/fstbin/fstminimizeencoded | ./kaldi-master/src/fstbin/fstpushspecial | \
    ./kaldi-master/tools/openfst-1.3.4/src/bin/fstarcsort --sort_type=ilabel > $lang/tmp/LG.fst || exit 1;
  ./kaldi-master/src/fstbin/fstisstochastic $lang/tmp/LG.fst || echo "[info]: LG not stochastic."
fi
...
if [[ ! -s $clg || $clg -ot $lang/tmp/LG.fst ]]; then
  ./kaldi-master/src/fstbin/fstcomposecontext --context-size=$N --central-position=$P \
   --read-disambig-syms=$lang/phones/disambig.int \
   --write-disambig-syms=$lang/tmp/disambig_ilabels_${N}_${P}.int \
    $lang/tmp/ilabels_${N}_${P} < $lang/tmp/LG.fst |\
    ./kaldi-master/tools/openfst-1.3.4/src/bin/fstarcsort --sort_type=ilabel > $clg
  ./kaldi-master/src/fstbin/fstisstochastic $clg  || echo "[info]: CLG not stochastic."
fi

if [[ ! -s $dir/Ha.fst || $dir/Ha.fst -ot $model  \
    || $dir/Ha.fst -ot $lang/tmp/ilabels_${N}_${P} ]]; then
  ./kaldi-master/src/bin/make-h-transducer --disambig-syms-out=$dir/disambig_tid.int \
...
if [[ ! -s $dir/HCLGa.fst || $dir/HCLGa.fst -ot $dir/Ha.fst || \
  ...
  if $remove_oov; then
    ...
    clg="./kaldi-master/src/fstbin/fstrmsymbols --remove-arcs=true --apply-to-output=true $lang/oov.int $clg|"
  fi
  ./kaldi-master/src/fstbin/fsttablecompose $dir/Ha.fst "$clg" | ./kaldi-master/src/fstbin/fstdeterminizestar --use-log=true \
    | ./kaldi-master/src/fstbin/fstrmsymbols $dir/disambig_tid.int | ./kaldi-master/src/fstbin/fstrmepslocal | \
     ./kaldi-master/src/fstbin/fstminimizeencoded > $dir/HCLGa.fst || exit 1;
  ./kaldi-master/src/fstbin/fstisstochastic $dir/HCLGa.fst || echo "HCLGa is not stochastic"
fi

if [[ ! -s $dir/HCLG.fst || $dir/HCLG.fst -ot $dir/HCLGa.fst ]]; then
  ./kaldi-master/src/bin/add-self-loops --self-loop-scale=$loopscale --reorder=true \
    $model < $dir/HCLGa.fst > $dir/HCLG.fst || exit 1;

  if [ $tscale == 1.0 -a $loopscale == 1.0 ]; then
    # No point doing this test if transition-scale not 1, as it is bound to fail.
    ./kaldi-master/src/fstbin/fstisstochastic $dir/HCLG.fst || echo "[info]: final HCLG is not stochastic."
  fi
fi
...
./kaldi-master/src/bin/am-info --print-args=false $model | grep pdfs | awk '{print $NF}' > $dir/num_pdfs

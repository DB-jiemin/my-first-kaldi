...
set -f;
if [ -f $tmpdir/phones_tmp ];then
	rm $tmpdir/phones_tmp
fi
for x in `cat $srcdir/silence_phones.txt`;do
	for y in "" "" "_B" "_E" "_I" "_S"; do
		echo -n "$x$y " >> $tmpdir/phones_tmp;
	done;
	echo >> $tmpdir/phones_tmp
...
cat $tmpdir/phones_tmp >$tmpdir/phone_map.txt
...
if [[ ! -z $phone_symbol_table ]]; then
  ...
  cat $dir/phones/disambig.txt | awk -v x=$start_symbol '{n=x+NR-1; print $1, n;}' > $tmpdir/phones_symbols_tmp
  ...
  BEGIN { while ((getline < f) > 0) { phones[$1] = $2; }} { print $1" "phones[$1]; }' | sort -k2 -g |\
    cat - $tmpdir/phones_symbols_tmp > $dir/phones.txt
else
...
if $silprob; then
  ...
  utils/make_lexicon_fst_silprob.pl $tmpdir/lexiconp_silprob.txt $srcdir/silprob.txt $silphone "<eps>" | \
     ./kaldi-master/tools/openfst-1.3.4/src/bin/fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
     --keep_isymbols=false --keep_osymbols=false |   \
     ./kaldi-master/tools/openfst-1.3.4/src/bin/fstarcsort --sort_type=olabel > $dir/L.fst || exit 1;
else
...
if $silprob; then
  utils/make_lexicon_fst_silprob.pl $tmpdir/lexiconp_silprob_disambig.txt $srcdir/silprob.txt $silphone '#'$ndisambig | \
     ./kaldi-master/tools/openfst-1.3.4/src/bin/fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
     --keep_isymbols=false --keep_osymbols=false |   \
     ./kaldi-master/src/fstbin/fstaddselfloops  $dir/phones/wdisambig_phones.int $dir/phones/wdisambig_words.int | \
     ./kaldi-master/tools/openfst-1.3.4/src/bin/fstarcsort --sort_type=olabel > $dir/L_disambig.fst || exit 1;
else
